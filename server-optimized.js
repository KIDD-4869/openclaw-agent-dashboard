...
// ── KeyedAsyncQueue：OpenClaw 风格的并发控制 ──
...
const fileQueue = new KeyedAsyncQueue();
...
async function getAgentConfig() {
  try {
    const cfg = await readFileSafe(path.join(OPENCLAW_DIR, 'openclaw.json'));
    const agents = {};
...
async function readRecentMessages(sessionFile, maxCount) {
  const TAIL_BYTES = 50 * 1024;
  const messages = [];
  if (!sessionFile) return messages;
  
  return fileQueue.enqueue(sessionFile, async () => {
    try {
...
async function loadSessions() {
  ...
  let dirs;
  try { dirs = await fs.promises.readdir(agentsDir); } catch { return results; }

  for (const agentId of dirs) {
    const sessFile = path.join(agentsDir, agentId, 'sessions', 'sessions.json');
    let store;
    try {
      store = await readFileSafe(sessFile);
    } catch {
      continue;
    }
    ...
    for (const [key, s] of Object.entries(store)) {
      ...
      const recentMessages = await readRecentMessages(s.sessionFile, 3);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Using KeyedAsyncQueue for concurrent file access protection`);
